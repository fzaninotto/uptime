...
DateNavigation.prototype.redrawPeriods = function() {
    ...
    var periods = '<div class="btn-group btn-group-xs">';
    if (this.interval.beginsAfterOrigin()) {
        periods += '<a class="btn btn-default" data-type="' + type + '" data-date="' + this.interval.getPreviousDate() + '">&lt;</a>';
    } else {
        periods += '<a class="btn btn-default" disabled="disabled">&lt;</a>';
    }
    ...
    while (d.valueOf() < end.valueOf()) {
        if (d.valueOf() < Date.now() && d.clone().endOf(subtype).valueOf() > this.interval.origin && !this.interval.isMaxZoom()) {
            periods += '<a class="btn btn-default ' + subtype + ' nb' + end.date() + '" data-type="' + subtype + '" data-date="' + d.valueOf() + '" title="' + this.tooltipForPeriod(d, subtype) + '">' + this.titleForPeriod(d, subtype) + '</a>';
        } else {
            periods += '<a class="btn btn-default ' + subtype + ' nb' + end.date() + '" disabled="disabled">' + this.titleForPeriod(d, subtype) + '</a>';
        }
    ...
    if (this.interval.endsBeforeNow()) {
        periods += '<a class="btn btn-default" data-type="' + type + '" data-date="' + this.interval.getNextDate() + '">&gt;</a>';
    } else {
        periods += '<a class="btn btn-default" disabled="disabled">&gt;</a>';
    }
    periods += '</div>';
...
DateNavigation.prototype.adjustWidth = function() {
    var navigationWidth = $('#dateNavigation .periods .btn-group').width() - 2*NAV_BTN_WIDTH;
