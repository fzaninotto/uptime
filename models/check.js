...
var Check = new Schema({
  ...
  maxTime: {
    type: Number,
    default: 1500
  }, // time under which a ping is considered responsive
  alertTreshold: {
    type: Number,
    default: 1
  }, // nb of errors from which to trigger a new CheckEvent
  errorCount: {
    type: Number,
    default: 0
...
Check.methods.removeStats = function (callback) {
  ...
  async.parallel([
    function (cb) {
      CheckHourlyStat.remove({
        check: self._id
      }, cb);
    },
    function (cb) {
      CheckDailyStat.remove({
        check: self._id
      }, cb);
    },
    function (cb) {
      CheckMonthlyStat.remove({
        check: self._id
      }, cb);
    },
    function (cb) {
      CheckYearlyStat.remove({
        check: self._id
      }, cb);
    }
  ], callback);
...
var statProvider = {
  'hour': {
    model: 'Ping',
    beginMethod: 'resetHour',
    endMethod: 'completeHour'
  },
  'day': {
    model: 'CheckHourlyStat',
    beginMethod: 'resetDay',
    endMethod: 'completeDay',
    duration: 60 * 60 * 1000
  },
  'month': {
    model: 'CheckDailyStat',
    beginMethod: 'resetMonth',
    endMethod: 'completeMonth',
    duration: 24 * 60 * 60 * 1000
  },
  'year': {
    model: 'CheckMonthlyStat',
    beginMethod: 'resetYear',
    endMethod: 'completeYear'
  }
};
...
Check.methods.getStatsForPeriod = function (period, begin, end, callback) {
  ...
  var stream = this.db.model(periodPrefs['model']).find(query).sort({
    timestamp: -1
  }).stream();
...
Check.methods.populateFromDirtyCheck = function (dirtyCheck, pollerCollection) {
  ...
  this.daysOfWeek = Check.methods.verifyDaysOfWeek(dirtyCheck);
  this.startHour = dirtyCheck.startHour;
  this.stopHour = dirtyCheck.stopHour;
...
Check.methods.verifyDaysOfWeek = function(data) {
  let listDays = [];
