...
Monitor.prototype.pollCheck = function(check, callback) {
  ...
  } catch (unkownPollerError) {
    return self.createPing(unkownPollerError, check, now, 0, {}, callback);
  }
  var pollerCallback = function(err, time, res) {
    if (err) {
      return self.createPing(err, check, now, time, {}, callback);
    }
    try {
      var details = Poller.getDetailsForPollAndCheck(res, check);
      self.createPing(null, check, now, time, details, callback);
    } catch (error) {
      return self.createPing(error, check, now, time, {}, callback);
    }
  };
  try {
    p = new Poller(check.url, this.config.timeout, pollerCallback);
  } catch (incorrectPollerUrl) {
    return self.createPing(incorrectPollerUrl, check, now, 0, {}, callback);
  }
