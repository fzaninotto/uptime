...
Monitor.prototype.pollCheck = function(check, callback) {
  ...
  let canExecute = Monitor.prototype.canExecute(check);
  if (canExecute) {
    p.poll();
  }
};

Monitor.prototype.canExecute = function(check) {
  let dateNow = new moment();
  let today = dateNow.day();
  let format = 'hh:mm';
  let start = moment(check.startHour, format);
  let stop = moment(check.stopHour, format);
  let now = moment(dateNow._d.getHours() + ":" + dateNow._d.getMinutes(), format);
  let validateHour = moment(now).isAfter(start) && moment(now).isBefore(stop);
  let containDay = check.daysOfWeek.includes(today);

  if (containDay && validateHour) {
    return true;
  } else {
    return false;
  }
