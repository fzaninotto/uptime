...
Monitor.prototype.findChecksNeedingPoll = function(callback) {
  ...
  http.get(api, function(res) {
    if (res.statusCode != 200) {
      return callback(new Error(self.config.apiUrl + '/check/needingPoll resource responded with error code: ' + res.statusCode));
    }
    var body = '';
    ...
    callback(new Error(self.config.apiUrl + '/check/needingPoll resource not available: ' + e.message));
  });
};
