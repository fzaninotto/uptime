...
Monitor.prototype.pollCheck = function(check, callback) {
  ...
  p = poller.createPoller(check.url, function(time, error) {
    self.createPing(check, false, time, error, callback);
    //check.setLastTest(false).save();
  }, function(time) {
    self.createPing(check, true, time, null, callback);
...
Monitor.prototype.createPing = function(check, status, time, error, callback) {
  status = status ? 'true' : 'false';
  var postData = 'checkId=' + check._id + '&status=' + status + '&time=' + time + '&name=' + this.config.name + '&error=' + error;
