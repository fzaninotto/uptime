...
Monitor.prototype.pollCheck = function(check, callback) {
  ...
  try {
    ...
    return self.createPing(check, err, now, 0, callback);
  }
  var p = new Poller(check.url, this.config.timeout, function(err, time, body) {
    if (!check.match || err) {
      return self.createPing(check, err, now, time, callback);
    }
    var match = check.match.match(new RegExp('^/(.*?)/(g?i?m?y?)$'));
    try {
      var regexp = new RegExp(match[1], match[2]);
      if (!body.match(regexp)) {
        err = new Error('Response body does not match pattern ' + check.match);
      }
    } catch (e) {
      err = new Error('Malformed pattern in schek configuration: ' + check.match);
    }
    self.createPing(check, err, now, time, callback);
  });
