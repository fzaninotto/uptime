...
/**
 * Prepares Content-Type and Content-Length HTTP headers and writes request content.
 *
 * @api   private
 */
BaseHttpPoller.prototype.prepareRequestWithContent = function() {
      var contentType = this.request.getHeader('Content-Type') || 'application/x-www-form-urlencoded',
        contentLength = this.request.getHeader('Content-Length') || Buffer.byteLength(this.http_body);
    this.request.setHeader('Content-Type', contentType);
    this.request.setHeader('Content-Length', contentLength);
    this.request.write(this.http_body);
};
