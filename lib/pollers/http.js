...
HttpPoller.prototype.setUserAgent = function(userAgent) {
  ...
  this.target.headers['User-Agent'] = this.userAgent;
};
...
HttpPoller.prototype.onResponseCallback = function(res) {
  ...
  if (statusCode == 301 || statusCode == 302) {
    ...
    if (!target.protocol) {
      ...
      var httpPoller = new HttpPoller('http://' + this.target.hostname + res.headers.location, this.timeout, this.callback);
      httpPoller.setUserAgent(this.userAgent);
      httpPoller.poll();
      return;
    }
    switch (target.protocol) {
      case 'http:':
        ...
        var httpPoller = new HttpPoller(target, this.timeout - elapsedTime, this.callback);
        httpPoller.setUserAgent(this.userAgent);
        httpPoller.poll();
        break;
      case 'https:':
        ...
        httpsPoller.setUserAgent(this.userAgent);
        httpsPoller.poll();
