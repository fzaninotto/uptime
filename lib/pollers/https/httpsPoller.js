...
HttpsPoller.prototype.poll = function(secure) {
  ...
  var loadFromPath = function (filePath) {
    try {
  ...
  // load key and cert from file
  if (this.target.keyPath && this.target.certPath) {
      this.target.key = loadFromPath(this.target.keyPath);
      this.target.cert = loadFromPath(this.target.certPath);
  }
  secure = typeof secure !== 'undefined' ? secure : true;
  try {
    if (secure) {
      this.request = https.request(this.target, this.onResponseCallback.bind(this));
    } else {
  ...
  if (this.http_body) {
    this.prepareRequestWithContent();
  }
  this.request.end();
...
HttpsPoller.prototype.handleRedirectResponse = function(res) {
  ...
  if (!target.protocol) {
    ...
    Object.assign(this.target, url.parse('https://' + this.target.hostname + res.headers.location));
    this.poll(false);
    return;
  }
