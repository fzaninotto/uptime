...
QosAggregator.prototype.createTagStatFromPingAggregationDocument = function(document, callback) {
  ...
  Check.findForTag(document._id, function(err, checks) {
    ...
    intervalBuilder.build(document.start, document.end, function(err2, outages, downtime) {
...
QosAggregator.prototype.createCheckDailyStats = function(documents, callback) {
  ...
  async.forEach(documents, function(document, next) {
    ...
    intervalBuilder.build(document.start, document.end, function(err, outages, downtime) {
      ...
      stat.outages = outages;
      stat.downtime = downtime;
      stat.availability = (day - downtime) / day;
...
QosAggregator.prototype.updateMonthlyQos = function(now, callback) {
  ...
  async.series([
    async.apply(this.updateMonthlyQosStats.bind(this), start, end, CheckDailyStat, CheckMonthlyStat, "$check", true),
    async.apply(this.updateMonthlyQosStats.bind(this), start, end, TagDailyStat, TagMonthlyStat, "$name", false),
  ], callback);
}

QosAggregator.prototype.updateMonthlyQosStats = function(start, end, dailyStatModel, monthlyStatModel, groupIdentifier, includeTags, callback) {
...
QosAggregator.prototype.updateYearlyQos = function(now, callback) {
  var start = TimeCalculator.resetYear(now);
  var end   = TimeCalculator.completeYear(now);
  var CheckMonthlyStat = require('../models/checkMonthlyStat');
  var CheckYearlyStat  = require('../models/checkYearlyStat');
  var TagMonthlyStat = require('../models/tagMonthlyStat');
  var TagYearlyStat  = require('../models/tagYearlyStat');
  async.series([
    async.apply(this.updateYearlyQosStats.bind(this), start, end, CheckMonthlyStat, CheckYearlyStat, "$check", true),
    async.apply(this.updateYearlyQosStats.bind(this), start, end, TagMonthlyStat, TagYearlyStat, "$name", false),
  ], callback);
}

QosAggregator.prototype.updateYearlyQosStats = function(start, end, monthlyStatModel, yearlyStatModel, groupIdentifier, includeTags, callback) {
  callback = this.getCallback(callback);
  var key = groupIdentifier.substr(1);
  var month = end - start;
  var pipeline = [
