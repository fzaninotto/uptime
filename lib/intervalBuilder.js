...
IntervalBuilder.prototype.build = function(begin, end, callback) {
  ...
  async.auto({
    ...
    interval: ['initialState', function(next) { self.buildIntervalsForPeriod(begin, end, next); }],
    check: ['initialState', 'interval', function (next) {self.getCurrentCheck(next)}],
    duration: ['initialState', 'interval', 'check', function (next, results) { self.calculateDuration(results.check, begin, end, next); }],
    downtime: ['initialState', 'interval', 'check', 'duration', function (next, results) { self.calculateDowntime(results.check, begin, end, next); }]
...
IntervalBuilder.prototype.updateCurrentState = function(timestamp) {
  ...
  if (currentState !== this.currentState) {
    var intervalCompleted = this.completeCurrentInterval(timestamp);
    this.currentInterval = currentState == 1 ? [] : [timestamp];
    if(intervalCompleted || this.currentState === null) {
