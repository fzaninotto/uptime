...
IntervalBuilder.prototype.build = function(begin, end, callback) {
  ...
  async.series([
    ...
    function(next) { self.calculateDuration(begin, end, next); }
  ], function(err) {
    if (err) return callback(err);
    self.completeCurrentInterval(end);
    var pausedTime = 0;
    self.intervals.forEach(function(interval) {
      if (interval[2] == PAUSED) {
        pausedTime += interval[1] - interval[0];
      }
    });
    return callback(null, self.intervals, self.downtime, self.duration - pausedTime);
  });
};
...
IntervalBuilder.prototype.calculateDuration = function(begin, end, callback) {
  if (this.isMultiTarget) {
    // it's a tag - no other way
    return callback(end - begin);
  }
  Check.findOne({ _id: this.objectIds[0] }, function(err, check) {
    if (err) return callback(end - begin);
    begin = Math.max(begin, check.firstTested);
    return callback(end - begin);
  });
};
